...
$(function(){
	campaign_advertising_creat2.GetProductList('1');
})
campaign_advertising_creat2.GetProductList=function(num){
        ...
        success:function(data){
        	if(data.result==1){
        		if(data.data.goods.length>0){
        			data.data.minrecoder=(parseInt(num)-1)*parseInt(pageSize)+1;
	        		data.data.maxrecoder=(parseInt(num)-1)*parseInt(pageSize)+data.data.goods.length;
	        		data.data.totalPages=Math.ceil(data.data.totalRecords/pageSize)
	        		var html=template(document.getElementById('product_show_conatiner-tpl').innerHTML,data);
					document.getElementById('product_show_conatiner').innerHTML = html;
					campaign_advertising_creat2.Pagination(data.data, num, campaign_advertising_creat2.GetProductList);
        		}else{
        			data.data.minrecoder=0;
	        		data.data.maxrecoder=0;
        			data.data.goods=[];
	        		$('.order-history-pagination').addClass('hide');
	        		var html=template(document.getElementById('product_show_conatiner-tpl').innerHTML,data);
					document.getElementById('product_show_conatiner').innerHTML = html;
        		}
...
campaign_advertising_creat2.Pagination= function (alldata, num,callback_function) {
    //分页
...
//添加自己的关键词
campaign_advertising_creat2.AddOwnKeyWords=function(){
...
//数据提交
campaign_advertising_creat2.CreateADGroupAuto=function(){
	...
	bid=campaign_advertising_creat2.FormatPrice(bid);
	if(!bid){
		alert('价格格式错误！');
		return false;
	}
	...
	$('#selected-keywords-table tbody tr').each(function(){
		...
		let kbid=campaign_advertising_creat2.FormatPrice(keywordbid);
		if(!kbid){
			alert('价格格式错误！');
			return false;
		}else{
			keywordbid=campaign_advertising_creat2.FormatPrice(keywordbid);
			let current={
				keyword:keyword,
				match:match,
				bid:keywordbid
...
//处理输入的价格
campaign_advertising_creat2.FormatPrice=function(price){
	let flag=1;
	let bid='';
