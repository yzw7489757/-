//var ptUrl='http://192.168.2.9:15000';
//if(document.referrer.indexOf(document.domain) == -1 && document.referrer.indexOf(ptUrl) == -1){
//    location.href=ptUrl; 
//}
var inputctr = {};
inputctr.public = {};
//亚马逊请求接口
var baseUrl='http://192.168.2.168:8006/QAMZNAPI.asmx';
// http://192.168.2.164:8096/QAMZNAPI.asmx
localStorage.getItem('baseUrl') ? baseUrl = localStorage.getItem('baseUrl') : baseUrl='http://192.168.2.168:8006/QAMZNAPI.asmx';
//任务请求接口地址
...
inputctr.public.LoadLang=function(){
    ...
    $("#languageSwitcher").change(function(){ 
        inputctr.public.setCookie("userLanguage",$(this).val());
...
inputctr.public.checkLogin=function(){
    ...
    var amazonId = inputctr.public.getCookie('amazon_userid');
	if(!amazonId || (SuccessAccount != StudentAccount) || (amazonId == 'undefined')){
        ...
        orgID = inputctr.public.getCookie('orgid');
        classID = inputctr.public.getCookie('classid');
        className = inputctr.public.getCookie('classname');
        orgID = inputctr.public.getCookie('orgid');
        userid = inputctr.public.getCookie('userid');
        account = inputctr.public.getCookie('account');
...
//select绑定国家
inputctr.public.selectCountry=function(fun){
...
//公用的ajax方法
inputctr.public.AjaxMethods=function(get_or_post,request_url,request_data,success_callback,error_callback,parmata){
	$.ajax({
        type:get_or_post,
        url:request_url,
        data:request_data,
        ...
        success:function(data){
        	if(!parmata){
        		...
        		success_callback(data,parmata)
        	}
          
        },
        error:function(XHR){
          error_callback(XHR,request_url)
        }
	});
}
