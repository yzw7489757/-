...
inputctr.addProductUploadfileQueued=function(uploader,imguploader,file){
    ...
    // $('#main-image-no-image').hide();
    //  var $li = $(
    //             '<div id="' + file.id + '" class="file-item thumbnail"><div class="close" onclick="inputctr.removeProductIng(this)"><i class="glyphicon glyphicon-remove"></i></div>' +
    //                 '<img>' +
    //                 '<div class="info">' + file.name + '</div>' +
    //             '</div>'
    //             ),
    //      $img = $li.find('img');
    //     $(imguploader).parents('#main-image-div').find('#main-image-has-image').html($li);
    //       // 创建缩略图
    //     // 如果为非图片文件，可以不用调用此方法。
    //     // thumbnailWidth x thumbnailHeight 为 预览图的宽高
    //     var thumbnailWidth=146;
    //     var thumbnailHeight=123;
