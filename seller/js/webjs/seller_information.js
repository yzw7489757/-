$(function () {
    ...
    $.post(baseUrl + '/GetSellerRegister2', { userid: amazon_userid }, function (data) {
        if (data) {
            inputctr.public.SellerRegisterLoadingRemove();
            inputctr.public.judgeBegaintask('1004');
        }
        if (data.step < '2') {
        ...
        if (data.result == 1) {
            ...
            if (inputctr.public.isChinaphone(country_phone_select.val())) {
                ...
                country_phone_select.attr('disabled', true);
                var language = inputctr.public.getCookie('userLanguage');
                var VerifyText = $('.a-verify-inline').addClass('a-alert-verify-success').removeClass('hide').find('.a-alert-content');
                if (!language || language == 'zh') {
                    VerifyText.text('验证成功');
                }
                if (language == 'en') {
                    VerifyText.text('Verification successful');
                }
                business_submit.attr('disabled', false);
            }
        } else {
    ...
    store_name.change(function () {
        ...
        var that = $(this);
        $.post(baseUrl + '/GetShopName', { userid: amazon_userid, shopname: shopName }, function (data) {
            if (data) {
                inputctr.public.SellerRegisterLoadingRemove();
            }
            if (data.result == 1) {
    ...
    business_submit.click(function () {
        console.log(UpdateInformation, address_id)
        ...
        inputctr.public.judgeRecodertask('1004', '提供卖家的具体信息开始');
        inputctr.public.SellerRegisterLoading();
        ...
        $.post(baseUrl + '/SellerRegister2', seller_information, function (data) {
            ...
            if (data.result == 1) {
                inputctr.public.judgeFinishtask('1004', link);
            } else {
                alert(decodeURIComponent(data.error))
            }
        }, 'json')
