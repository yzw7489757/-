$(function () {
  ...
  function changeImgclick(ulName, classNameHover, classNameClick, input_click) {
    $(`${ulName} li`).click(function (e) {
      ...
      $(ulName)
        .find(".a-icon-radio")
        .removeClass(`${classNameClick} ${input_click}`); //先删除所有的
      // $(ulName)
      //   .find("input")
      ...
      $(ulName)
        ...
        .eq(index)
        .find(".a-icon-radio")
        .addClass(`${classNameClick} ${input_click}`);
      $(ulName)
        .find(".checkInput")
  ...
  $(".emailPreference li").click(function (e) {
    ...
    if (email_format === 0) {
      email_format = 1;
      $(".a-icon-checkbox").addClass("checkbox_click input_click");
    } else {
      email_format = 0;
      $(".a-icon-checkbox").removeClass("checkbox_click input_click");
    }
  });
  // 默认的自动批准退货申请规则
  ...
  $(".saveBtn").click(function () {
   ...
   $('.successInfo').hide()
    if (hasDefaultAddress===1/*email_format === 1*/) {
  ...
  // 配送设置（初始化配送地址）  GetAddressList 未确定是否是这个方法
  $.ajax({
  ...
  $(".cancel_save_settings").find(".cancelBtn").click(function(){
    window.location.reload();
  });
  $(".cancel_save_settings").find(".saveBtn").click(function(){
