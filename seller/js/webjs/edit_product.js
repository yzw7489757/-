$(function(){
    ...
    function vitalInfo(vitalInfo,goodsInfo){
        ...
        vital_segment.find('input.ProductName').val(decodeURIComponent(goodsInfo.product_name));
        ...
        $.each(list, function(i, val) {
            ...
            var tr_html = '<tr class="raw" sku_id ='+val.sku_id+'>'+
                                ...
                                '<td class="attribute '+size_show+'" data-attribute-name="size_name">'+
                                    '<span class="variation-key">'+decodeURIComponent(val.size)+'</span>'+
                                '</td>'+
                                '<td class="attribute '+color_show+'" data-attribute-name="color_name">'+
                                    '<span class="variation-key">'+decodeURIComponent(val.color)+'</span>'+
                                '</td>'+
    ...
    function Images(productsImgs,mainImg){
        ...
        if(mainImg){
            $(imageUploadWidget[0]).attr('img_path',decodeURIComponent(mainImg)).find('.chooseImageButton').addClass('hide').siblings('.removeImageButton').removeClass('hide');
            $(imageUploadWidget[0]).find('.previewImage').attr('src',decodeURIComponent(mainImg)).removeClass('hide').siblings('.noFileSelectedImage').addClass('hide');
        }
        if(productsImgs.length){
            for(var m=1;m<imageUploadWidget.length;m++){
                ...
                $(imageUploadWidget[m]).attr('img_path',decodeURIComponent(mainImg)).find('.chooseImageButton').addClass('hide').siblings('.removeImageButton').removeClass('hide');
                $(imageUploadWidget[m]).attr('img_path',decodeURIComponent(productsImgs[m-1].Column1)).find('.previewImage').attr('src',decodeURIComponent(productsImgs[m-1].Column1)).removeClass('hide').siblings('.noFileSelectedImage').addClass('hide');
            }
        }
    ...
    function Description(depict){
        ...
        Description_segment.find('#product_description').val(decodeURIComponent(depict.description));
    ...
    function bulletInit(target,bulletArr){
        add_n = (bulletArr.length > 5) ? 5 : bulletArr.length;
        ...
        if($(target).attr('class') == 'cpsia_cautionary_statement'){
            ...
            for(var t=0;t<input.length;t++){
                $(input[t]).val(decodeURIComponent(bulletArr[t]));
            }
            if(add_n >= 2 && add_n < 5){
  ...
  // 添加尺寸、颜色 input
    ...
    main_submit_button.click(function(){
        inputctr.public.SellerRegisterLoading();
        inputctr.public.judgeRecodertask('1209','调整自有库存商品数量开始');
        inputctr.public.judgeRecodertask('1210','调整当前正在售卖商品价格和运费开始');
        ...
        inputctr.public.judgeFinishtask('1209');
        inputctr.public.judgeFinishtask('1210');
        // Offer
        var Offer_segment = $('.Offer_segment');
