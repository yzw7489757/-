$(function(){
        ...
        inputctr.public.AjaxMethods('POST', baseUrl + '/GetReturnRequest', {json:JSON.stringify(info)}, function (data) {
            ...
            if(data.result == 1) {
                for(var i=0;i<data.data.length;i++){
                    var Authorize_status = (data.data[i].refund_status == 2) ? 'hide' : '';
                    var request_status = (data.data[i].refund_status >=3) ? 'hide' : '';
                    order_html += '<div class="order-detainls-container clearfix">'+
                                    ...
                                    '<div class="pull-left order-information-container3 clearfix">'+
                                        '<button class="highlighted-button pull-right '+Authorize_status+' '+request_status+'" onclick="javascript:location.href=\'authorize_request.html?fld_id='+data.data[i].fld_id+'\'">'+
                                            '<span class="highlighted-button-label button">Authorize request</span>'+
                                        '</button>'+
                                        '<button class="find find_on pull-right '+Authorize_status+' '+request_status+'" onclick="javascript:location.href=\'close_request.html?fld_id='+data.data[i].fld_id+'\'">'+
                                            '<span class="button-label button">Close request</span>'+
                                        '</button>'+
                                        '<button class="find find_on pull-right '+request_status+'" onclick="javascript:location.href=\'refund_order.html?fld_id='+data.data[i].fld_id+'\'">'+
    ...
    function  initPage(totalPage,currentPage) {
        ...
        if(totalPage >= 1){
           ...
           if((parseInt(currentPage) + parseInt(maxButtons) - 1) >=  totalPage){
                pageHtml += '';
           }else{
