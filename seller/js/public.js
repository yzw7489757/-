...
inputctr.public.RegisterStep=function(step){
    switch (step){
        case '0': window.location.href='seller_agreement.html';
          break;
        case '1': window.location.href='seller_information.html';
          break;
        case '2': window.location.href='billing_deposit.html';
          break;
        case '3': window.location.href='tax_information.html';
          break;
        case '4': window.location.href='product_information.html';
          break;
        case '5': window.location.href='product_category.html';
          break;
    }
}
...
inputctr.public.LoginOut=function(){
    ...
    LoginOut.click(function(){
        ...
        inputctr.public.clearCookie('LoginSuccess');
        setTimeout(function(){
...
inputctr.public.checkLogin=function(){
	...
	if(!inputctr.public.getCookie('LoginSuccess') || SuccessAccount != StudentAccount){
		var recodeUrl=encodeURIComponent(window.location.href);
		window.location.href="login.html?rediect="+	recodeUrl;
	}else{
		amazon_userid=inputctr.public.getCookie('LoginSuccess');
	}
}
//js获取地址栏参数
...
inputctr.public.TaskListAppend=function(){
    let language = inputctr.public.getCookie('lang');
    if(!language || language=='zh'){
        var TaskText = "任务";
    }
    if(language=='en'){
    ...
    $.post(TaskUrl+'/TrainingTaskUserList',commitdata,function(response){
        ...
        if(data.list.length>0){
            ...
            let content='<div class="modal" id="taskContainer"><div class="modal-dialog">'
                    ...
                    +'<button type="button" class="close"><span class="close_modal">&times;</span></button>'
